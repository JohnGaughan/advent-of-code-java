...
/**
 * <p>
 * <a href="https://adventofcode.com/2018/day/4">Year 2018, day 4</a>. This problem presents a quantity of log entries
 * describing different guard IDs and when they sleep on the job. We need to parse the data, measure metrics, and
 * multiply two numbers together based on the contents of that data.
 * </p>
 * <p>
 * Most of the work is in parsing the input into a usable format. From there we just need to search through the guards'
 * data for values matching whatever metrics are applicable to the part of the problem.
 * </p>
 * <p>
 * Copyright (c) 2021 John Gaughan
 * </p>
...
public final class Year2018Day04 {
  ...
  public long calculatePart1() {
    final Guard mostAsleep = getInput().stream().max((a, b) -> Integer.compare(a.asleep, b.asleep)).get();
    return mostAsleep.id * mostAsleep.mostAsleepMinute;
  }

  public long calculatePart2() {
    Guard selected = null;
    for (final Guard guard : getInput()) {
      if (selected == null) {
        selected = guard;
      }
      else {
        if (guard.timesAsleep > selected.timesAsleep) {
          selected = guard;
        }
      }
    }
    return selected == null ? 0 : selected.id * selected.mostAsleepMinute;
  }

  /** Get the input data for this solution. */
  private Collection<Guard> getInput() {
    try {
      final Event[] events =
        Files.readAllLines(Utils.getInput(2018, 4)).stream().map(Event::new).sorted().toArray(Event[]::new);
  ...
  implements Comparable<Event> {
    ...
    Event(final String input) {
      month = Integer.parseInt(input.substring(6, 8));
