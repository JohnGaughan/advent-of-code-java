  ...
  public void execute(final State state) {
    ...
    while (0 <= state.ip && state.ip < state.instructions.length) {
      ...
      if (state.instructions[state.ip].op == OpCode.cpy) {
        final int value;
        if (state.instructions[state.ip].args[0] instanceof Integer) {
          value = ((Integer) state.instructions[state.ip].args[0]).intValue();
        }
        else {
          char srcReg = ((Character) state.instructions[state.ip].args[0]).charValue();
          value = state.reg[srcReg - 'a'];
        }
        if (state.instructions[state.ip].args[1] instanceof Character) {
          char destReg = ((Character) state.instructions[state.ip].args[1]).charValue();
          state.reg[destReg - 'a'] = value;
        }
        ++state.ip;
      }
      else if (state.instructions[state.ip].op == OpCode.dec) {
        if (state.instructions[state.ip].args[0] instanceof Character) {
          char reg = ((Character) state.instructions[state.ip].args[0]).charValue();
          --state.reg[reg - 'a'];
        }
        ++state.ip;
      }
        ...
        else if (state.instructions[state.ip].args[1] instanceof Integer) {
          state.ip += ((Integer) state.instructions[state.ip].args[1]).intValue();
        }
        else {
        ...
        final int argA;
        if (state.instructions[state.ip].args[0] instanceof Integer) {
          argA = ((Integer) state.instructions[state.ip].args[0]).intValue();
        }
        else {
          char srcReg = ((Character) state.instructions[state.ip].args[0]).charValue();
        ...
        final int argB;
        if (state.instructions[state.ip].args[1] instanceof Integer) {
          argB = ((Integer) state.instructions[state.ip].args[1]).intValue();
        }
        else {
          char srcReg = ((Character) state.instructions[state.ip].args[1]).charValue();
          argB = state.reg[srcReg - 'a'];
        }
        if (state.instructions[state.ip].args[2] instanceof Character) {
          char destReg = ((Character) state.instructions[state.ip].args[2]).charValue();
