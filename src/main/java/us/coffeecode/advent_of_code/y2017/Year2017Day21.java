...
/**
 * <p>
 * <a href="https://adventofcode.com/2017/day/21">Year 2017, day 21</a>. This problem asks us to transform a grid using
 * rules, with the two parts varying only by the number of iterations.
 * </p>
 * <p>
 * There are a couple of optimizations here which also simplify the solution a bit. First, we use a wrapper around a
 * boolean 2D array. This simplifies array access a bit compared to 3D or 4D arrays. It also allows overriding
 * hashCode() and equals() so it can be used as a key in a HashMap. Next, we can front-load the matrix operations to the
 * time of loading input, which means we only need to perform them once per transformation. Then the main loop does not
 * need to modify any inputs, it can simply compare them.
 * </p>
 * <p>
 * Performance is still a little slow for part two, but further optimizations would require identifying patterns and
 * skipping iterations. It appears this is theoretically possible, but not worth the effort.
 * </p>
 * <p>
 * Copyright (c) 2021 John Gaughan
 * </p>
...
public final class Year2017Day21 {
  ...
  private long calculate(final int iterations) {
    final Mappings maps = getInput();
    boolean[][] grid = INITIAL_STATE;
    for (int i = 0; i < iterations; ++i) {
      grid = iteration(grid, maps);
    }
  ...
  private boolean[][] iteration(final boolean[][] grid, final Mappings maps) {
    final int preSize;
    final int postSize;
  ...
  private Mappings getInput() {
    try {
      final Mappings mappings = new Mappings();
      for (final String line : Files.readAllLines(Utils.getInput(2017, 21))) {
        ...
        if (key[0][0] && key[0][1] && !key[1][0] && !key[1][1]) {
          Thread.yield();
        }
  ...
  private static final class BooleanGrid {
    ...
    public boolean equals(final Object obj) {
      ...
      else if (!(obj instanceof BooleanGrid)) {
        return false;
      }
      else {
        return Arrays.deepEquals(grid, ((BooleanGrid) obj).grid);
      }
    }
  ...
  private static final class Mappings {
    ...
    final Map<BooleanGrid, BooleanGrid> threes = new HashMap<>(2048);
  }
