...
/**
 * <p>
 * <a href="https://adventofcode.com/2017/day/18">Year 2017, day 18</a>. This problem is an exercise in a assembly
 * language, but with a twist. Part one is a regular assembly problem seen many times in Advent of Code. However, part
 * two changes it by adding a second program that runs in parallel, with both programs communicating with each other.
 * </p>
 * <p>
 * There are two approaches that make sense here. First, single-threaded. Run one program until it blocks on input, then
 * switch to the other program until it also blocks. Then switch back to the first program, and repeat until both
 * programs are blocked. Note that all program inputs do result in deadlock, confirmed by the puzzle's authors. The
 * second approach is an actual threaded solution, which ended up being simpler. Using thread primitives introduced in
 * Java 5, it is trivial to sit and wait for both threads to complete. With some carefully-written Callable tasks, we
 * can guarantee that even in a "deadlock" scenario, the threads are able to detect this and quit. Given how simple and
 * fast the code is to execute assembly, setting a low timeout for polling input ensures that a thread does not wait too
 * long before giving up and killing itself. However, it is always possible that this might not work correctly on a
 * slower system than the one I use.
 * </p>
 * <p>
 * Copyright (c) 2021 John Gaughan
 * </p>
...
public final class Year2017Day18 {
  ...
  private List<Instruction> getInput() {
    try {
      return Files.readAllLines(Utils.getInput(2017, 18)).stream().map(Instruction::new).collect(Collectors.toList());
    }
