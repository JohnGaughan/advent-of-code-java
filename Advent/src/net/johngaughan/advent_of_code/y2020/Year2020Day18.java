...
/**
 * <p>
 * <a href="https://adventofcode.com/2020/day/18">Year 2020, day 18</a>. This problem is an exercise in parsing a string
 * into a mathematical expression, where the expression only has numerical constants, addition, multiplication, and
 * parenthesis. Part one has the rule that there is no operator precedence: addition and multiplication occur left to
 * right. Part two switches it up by stating that addition has precedence.
 * </p>
 * <p>
 * There are multiple ways to handle this problem, and I opted to use two different algorithms. For part one, I wrote a
 * <a href="https://en.wikipedia.org/wiki/Recursive_descent_parser">recursive-descent</a> left-associative parser. Given
 * part one has no operator precedence, this is a fairly simple implementation that can greedily consume tokens and add
 * them to Expression objects without more advanced logic.
 * </p>
 * <p>
 * For part two, I implemented a <a href="https://en.wikipedia.org/wiki/Shunting-yard_algorithm">shunting-yard
 * algorithm</a> that handles operator precedence: since it has to manage extra data structures anyway, it did not add
 * any measurable complexity. After it runs, I reverse the output which converts it to
 * <a href="https://en.wikipedia.org/wiki/Reverse_Polish_notation">RPN</a>. I opted not to generalize the solution for
 * part two and apply it to part one, simply so I have an example of both approaches.
 * </p>
 * <p>
 * In both cases, I parse input tokens into Expression objects. This ends up being a highly-unbalanced binary tree, but
 * is an interesting intermediate form that aids in debugging.
 * </p>
 * <p>
 * Copyright (c) 2020 John Gaughan
 * </p>
...
public final class Year2020Day18 {
  ...
  public long calculatePart1() {
    return getInput(s -> recursiveDescent(s)).stream().mapToLong(Expression::evaluate).sum();
  }

  public long calculatePart2() {
    return getInput(s -> shuntingYard(s)).stream().mapToLong(Expression::evaluate).sum();
  }

  /** Get the input data for this solution. */
  private List<Expression> getInput(final Function<String, Expression> algorithm) {
    try {
      return Files.readAllLines(Utils.getInput(2020, 18)).stream().map(s -> parseLine(s, algorithm)).collect(
        Collectors.toList());
    }
