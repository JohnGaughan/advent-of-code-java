 ...
 */
public final class Year2020Day2Part1
implements AdventProblem {
  ...
  implements Predicate<String> {
    ...
    public boolean test(final String t) {
      ...
      // Get the min and max character count, and the character to test.
      final int split1 = pieces[0].indexOf('-');
      final int split2 = pieces[0].indexOf(' ', split1);
      final int minCount = Integer.parseInt(pieces[0].substring(0, split1));
      final int maxCount = Integer.parseInt(pieces[0].substring(split1 + 1, split2));
      final char character = pieces[0].charAt(split2 + 1);

      // Count how many times the character appears.
      final int count = (int) pieces[1].chars().filter(c -> c == character).count();
